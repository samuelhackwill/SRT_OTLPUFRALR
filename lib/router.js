Router.route('/', {
  ...
  action: function () {
    if(this.ready()) {
      balisesVue = ["NL_SALM", "NL_CYCLO", "NL", "FR_CYCLO", "EN_SALM", "EN_CYCLO", "EN", "FR_INTR", "NL_INTR", "DE_INTR", "EN_INTR", "DE_SALM", "DE_CYCLO", "FR_SALM", "DE"]
      modeSpectacle = getSuperGlobal("modeSpectacle");
      console.log("modeSpectacle??", modeSpectacle);
      //checkons si il y a le cookie
      checkCookie = cookies.get("get_me_in");
      console.log("cookie get me in??", checkCookie)
      var checkCookieLan = cookies.get("user_lan");
      // checkons si y'a eu un clic sur la pastille parce qu'on dirait qu'un check cookie c'est pas réactif
      var checkPastilleClicked = Session.get('pastilleClicked')


      console.log("router - checkCookie??", checkCookie, cookies);
      if(modeSpectacle) {
        console.log("modeSpectacle activé en tout cas")
        if(null != checkCookie && undefined != checkCookie || checkPastilleClicked) {
          console.log("router - la personne a déjà cliqué sur la pastille une fois ajd");
          this.render('spectacle');
        } else this.render('spectacle');
      } 
      // boh si c'est pas le mode spectacle t'attends pis c'est tout
      else this.render('spectacle');
    } else {
      // si t'es pas prêt tu charges
      // tin ça serait malin de tout caler les chargements de cookie etc dans loading plutôt que dans waiting...
      this.render('loading');
    }
  },
